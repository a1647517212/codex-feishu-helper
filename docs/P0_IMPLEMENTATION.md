...
- `desktop_proxy` is now the only supported Feishu-origin new-thread path. The name is kept for compatibility, but the actual transport now uses direct `codex app-server` stdio rather than `codex app-server proxy`. In `desktop_ipc`, Feishu still controls ordinary Desktop through the Desktop IPC pipe without starting a separate runtime, but new-task entry points no longer rely on `start-conversation` as a product capability; they keep a retryable draft and guide `/tasks` claim flow instead. Low-level `start-conversation` / `thread/start` transport probes remain in the IPC client for protocol verification only. Follow-up controls route through Desktop owner follower IPC only after a real owner snapshot exists.
- When config explicitly prefers `desktop_proxy` but the direct `codex app-server` path is unavailable, `CodexClient.start()` now degrades transport startup to `desktop_ipc` instead of letting bridge startup fail outright. This only preserves claim/continue behavior for already opened Desktop threads; it does not re-enable Feishu-origin new-thread creation.
