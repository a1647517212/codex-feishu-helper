...
| 6.2 Claim existing Codex App task | Done | `/tasks`, `/claim <threadId>`, `[在飞书继续]`, summary, ignore, project assignment flows, and Codex App-only completion reminders list/read/bind existing app-server threads into dedicated Feishu task chats. | Real Codex Desktop session fidelity is limited to app-server visibility. |
...
| 15 Notification strategy | Done for personal use | Completion/failure/approval/status outbox, retry, history, global/project/session notification preferences, and background reminders for unbound Codex App-only completed/failed/interrupted threads are implemented. Approvals and status refresh bypass muting where appropriate. | @ user policy is not needed for current personal-use scope. |
...
| 23.2 Claim acceptance | Done | Claim existing thread, create a dedicated Feishu task chat, continue same Codex thread, summarize, ignore, background Codex App-only completion reminder, and button/command fallback are covered by tests. | Desktop GUI-only state remains outside current app-server boundary. |
...
| 44.7 Codex App interop | Partial | Existing idle app-server sessions can be discovered, reminded on completion when unbound, resumed, continued, and archived on completion. | Rollout import and Desktop GUI visibility not implemented. |
