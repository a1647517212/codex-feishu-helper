...
The project currently implements the main P0 bridge kernel: Feishu long-connection message transport by default, long-connection card callbacks, button-first hybrid interaction with message-command fallback for local-only deployments, optional HTTP callback fallback, official Feishu `reply_in_thread=true` task-topic creation with persisted `thread_id`, Codex app-server stdio transport, SQLite persistence, session binding, semantic events, projections, queueing, approvals, outbox retry, diagnostics, incoming-message dedupe, unclassified task assignment, and basic safety controls.

It does not yet implement the full product described by the design. The biggest remaining gaps are notification preferences, local detail pages, checkpointing, Desktop owner routing, live mirror/import, deep links, multi-machine support, team permissions, and commit/push/PR workflows. One important platform nuance remains: in a normal message-form group, `reply_in_thread=true` creates an official message thread/topic and returns a `thread_id`, but the group feed still looks like a normal group reply flow. For all tasks to appear in a full topic-mode feed, the chat itself must be `group_message_type=thread` or `chat_mode=topic`.
...
| Whole design document | Medium to high |
...
| 2 Core conclusion | Done | One Codex thread maps to one Feishu root/topic binding in `session_bindings`; task cards are sent with `reply_in_thread=true` and the returned `thread_id` is persisted. | Full topic-feed UX depends on the target group being `group_message_type=thread`. |
...
| 6.1 New task from Feishu | Done | New free-text message sends a bot-owned anchor message, replies with a task card using `reply_in_thread=true`, persists `rootMessageId/threadId`, then starts a Codex thread and turn. | In a normal group, the Feishu UI still displays the thread inside the group feed unless the group itself is switched to topic-message mode. |
| 6.2 Claim existing Codex App task | Done | `/tasks`, `/claim <threadId>`, `[在飞书继续]`, summary, ignore, and project assignment flows list/read/bind existing app-server threads into bot-owned Feishu task threads. | Real Codex Desktop session fidelity is limited to app-server visibility. |
| 6.3 Continue in task topic | Done | `findBindingByFeishuThreadId`, `findBindingByTopic`, `steerTurn`, `resumeThread`, `startTurn`, and busy queue route replies to the canonical bound Feishu thread. | Full active-turn semantics are still limited by available app-server behavior. |
...
| 7 Card and copy design | Partial | Console, new task, claim list, task status, approval, queue, diagnostics, history, unclassified, and project-assignment cards exist. | Checkpoint card and full visual polish are not complete. |
...
| 9 Project abstraction and mapping | Partial | Configured projects are stored; cwd/git-root/git-remote matching works across Windows slash styles; `/unclassified`, create-project, pick-project, assign-project, summary, and ignore flows exist. | Auto directory discovery and richer project settings UI are missing. |
...
| 19 MVP phase 1 | Done | Main loop works in code/tests and real Feishu long-connection group flow was verified; direct group messages without `@` create bot-owned task threads and start Codex turns. | Feishu group display depends on chat message mode. |
...
| 23.1 New task acceptance | Done | Feishu text can start Codex, create a Feishu reply thread, persist `thread_id`, update cards, and dedupe retried incoming messages. | Real file/Git verification depends on the task being executed by Codex. |
| 23.2 Claim acceptance | Done | Claim existing thread, create a Feishu task thread, continue same Codex thread, summarize, ignore, and button/command fallback are covered by tests. | Desktop GUI-only state remains outside current app-server boundary. |
| 23.3 Project mapping acceptance | Done for current scope | Config path prefix, Windows path normalization, Git root/remote rule matching, `/unclassified`, assign, and create-project flows exist. | Auto directory discovery remains future work. |
...
| 24 Risks and open questions | Partial | Major risks documented as current boundaries; live Feishu long-connection message flow was verified; card callbacks can be diagnosed/fixed for websocket; group topic-mode can be diagnosed. | Desktop validation and optional chat conversion still need operator confirmation/live coverage. |
...
| 42.1 P0 required | Mostly done | Engineering P0 items implemented, including long connection, websocket card callback diagnostics, official Feishu task threads, diagnostics, queue, outbox, approvals, project matching, unclassified flows, and incoming dedupe. | Runtime schema validation and notification preferences remain incomplete. |
...
direct group free text -> bot-owned anchor message + `reply_in_thread=true` task card + persisted `thread_id`
...
1. Decide whether to convert the target group to full topic-message mode (`group_message_type=thread`) or keep normal group mode with per-task reply threads.
...
4. Project discovery UI beyond current config/unclassified assignment flows.
