...
The project currently implements the main P0 bridge kernel: Feishu long-connection message transport by default, long-connection card callbacks, button-first hybrid interaction with message-command fallback for local-only deployments, optional HTTP callback fallback, one dedicated Feishu task chat per Codex task by default, optional `reply_in_thread=true` topic fallback, Codex app-server stdio transport, SQLite persistence, session binding, semantic events, projections, queueing, approvals, outbox retry, diagnostics, incoming-message dedupe, unclassified task assignment, and basic safety controls.

It does not yet implement the full product described by the design. The biggest remaining gaps are notification preferences, local detail pages, checkpointing, Desktop owner routing, live mirror/import, deep links, and multi-machine support. For personal use, team permission workflows are intentionally out of scope. One important platform nuance remains for fallback topic mode: in a normal message-form group, `reply_in_thread=true` creates an official message thread/topic and returns a `thread_id`, but the group feed still looks like a normal group reply flow. The default dedicated-chat mode avoids that UX limitation by creating a separate Feishu conversation per task.
...
| 6.4 Completion notification | Done | `turn/completed` inserts events, enqueues a status card, and sends a clean text result with `处理摘要` and `最终结论`. Extraction uses `thread/read`, completed items, and streamed delta fallback. | Notification preferences are not applied yet. |
...
| 13 Codex app-server integration | Partial | Stdio JSONL client, initialize, thread/turn methods, notifications, server requests, completed item and streamed delta extraction for final Feishu reports. | Full official runtime schema validation and all event types are not wired. |
...
| 16 Change and log display | Partial | Event/log cards, task status cards, and sanitized summaries exist. | Large output paging, local detail pages, and richer file-level detail are missing. |
...
| 19 MVP phase 2 | Partial | Queue view/cancel, task status/log helpers, and final result reporting exist. | Notification preferences and local detail pages are incomplete. |
...
| 37.3 Local execution summary | Partial | Personal local-task flow is implemented with concise summaries and event logs. | Richer file detail and local detail pages are still missing. |
...
| 40 Local detail page | Not started | No local detail page. | Needed for large output and richer task detail. |
...
| 42.1 P0 required | Mostly done | Engineering P0 items implemented, including long connection, websocket card callback diagnostics, dedicated Feishu task chats, topic fallback, diagnostics, queue, outbox, approvals, project matching, unclassified flows, incoming dedupe, and final report extraction. | Runtime schema validation and notification preferences remain incomplete. |
| 42.2 P1 enhancements | Partial | Queue view/cancel and basic history exist. | Most P1 items remain incomplete. |
...
1. Verify live dedicated-chat creation after Feishu app scopes are granted.
