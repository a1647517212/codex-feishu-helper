...
The project currently implements the main personal-use bridge: Feishu long-connection message transport by default, long-connection card callbacks, button-first hybrid interaction with message-command fallback, optional HTTP callback fallback, one dedicated Feishu task chat per Codex task by default, optional `reply_in_thread=true` topic fallback, ordinary Codex Desktop IPC transport, SQLite persistence, session binding, semantic events, projections, queueing, approvals, outbox retry, notification preferences, diagnostics with recovery, incoming-message dedupe, unclassified task assignment, Codex Desktop-only completion reminders, task search, local task detail pages, and non-Git workspace checkpoints for “本次影响”.

It still does not implement every advanced item described by the design. The biggest remaining gaps are Desktop-origin live mirror/import, richer first-run pairing, project directory discovery, and a stock-Desktop path for Feishu-origin new-thread creation when official proxy is unavailable. The current supported Desktop path creates real new Desktop threads only through `desktop_proxy`; `desktop_ipc` is now treated as claim/continue-only for product behavior, while low-level `start-conversation` / `thread/start` experiments remain transport probes. For personal use, team permission workflows are intentionally out of scope. Git-centric UX is not part of the current product surface; file impact and limited restore are derived from local workspace snapshots instead. One important platform nuance remains for fallback topic mode: in a normal message-form group, `reply_in_thread=true` creates an official message thread/topic and returns a `thread_id`, but the group feed still looks like a normal group reply flow. The default dedicated-chat mode avoids that UX limitation by creating a separate Feishu conversation per task.
...
| --- | --- | --- | --- |
| 1 Background and goal | Partial | Bridge controls ordinary Codex Desktop threads from Feishu messages/cards. Real long-connection `/codex` in group was verified without `@`. | Full goal says mature notifications and richer task lifecycle; advanced pieces remain incomplete. |
...
| 3 Design boundary | Partial | Uses ordinary Codex Desktop IPC and accepts eventual sync. | Desktop GUI mirror and richer active-turn semantics are not implemented. |
...
| 6.1 New task from Feishu | Partial | New free-text message in the main control group creates a dedicated Feishu task chat. When `desktop_proxy` is connected, it starts a real Codex thread/turn through direct `codex app-server`, binds the new chat, and updates the chat name with status. When the resolved connection is `desktop_ipc`, project selection keeps a retryable draft and guides `/tasks` claim flow instead of pretending a new thread exists. | Full end-to-end success still depends on the local `codex app-server` path being available on the current machine. |
| 6.2 Claim existing Codex Desktop task | Done | `/tasks`, `/claim <threadId>`, `[在飞书继续]`, summary, ignore, project assignment flows, and Codex Desktop-only completion reminders list/read/bind existing ordinary Desktop threads into dedicated Feishu task chats. | Real Codex Desktop session fidelity is still limited by the currently exposed IPC snapshot surface. |
| 6.3 Continue in task chat | Done | `findBindingByChatId`, `findBindingByFeishuThreadId`, `findBindingByTopic`, `steerTurn`, `resumeThread`, `startTurn`, and busy queue route replies to the bound task container; in `desktop_ipc` mode follower requests are routed to the ordinary Desktop owner client. | Full active-turn semantics are still limited by available Desktop IPC behavior. |
...
| 13 Codex Desktop integration | Partial | Desktop IPC client, Desktop Proxy client, initialize, thread/turn methods, notifications, server requests, completed item and streamed delta extraction for final Feishu reports. `desktop_proxy` is the supported new-thread path for Feishu; `desktop_ipc` observes ordinary Desktop snapshots and routes follower controls to the Desktop owner for existing threads. Low-level `start-conversation` / `thread/start` probes remain transport tests only. | Full official runtime schema validation and all event types are not wired. Desktop live refresh depends on the local Codex IPC pipe or proxy runtime being available. |
| 14 Feishu integration | Done for P0 | Long connection is default for messages and `card.action.trigger` callbacks; message commands cover fallback controls; HTTP callback is optional fallback; message parsing, card action parsing, send/reply/update APIs exist; `im/v1/chats` creates dedicated task chats; chat title update marks running/completed/failed; `reply_in_thread=true` remains fallback. | Dedicated-chat mode requires `im:chat:create` and `im:chat:update`; member-write permission is only needed for adding more users to existing task chats. Card buttons require app-side `card.action.trigger` long-connection subscription and publish. |
| 15 Notification strategy | Done for personal use | Completion/failure/approval/status outbox, retry, history, global/project/session notification preferences, and background reminders for unbound Codex Desktop-only completed/failed/interrupted threads are implemented. Approvals and status refresh bypass muting where appropriate. | @ user policy is not needed for current personal-use scope. |
...
| 18 Error handling | Partial | HTTP errors, Desktop IPC diagnostic status, protocol validation event, idempotent actions. | Expired approval and unavailable Desktop recovery cards are basic. |
...
| 23.1 New task acceptance | Partial | Feishu text can create a dedicated task chat, persist chat binding, update status title/cards, send final result, and dedupe retried incoming messages. Real thread creation is covered only when `desktop_proxy` is available; otherwise the current product keeps a retryable draft and guides claim flow. | Real file-level verification depends on the task being executed by Codex, and real new-thread creation still depends on local direct `codex app-server` availability. |
| 23.2 Claim acceptance | Done | Claim existing thread, create a dedicated Feishu task chat, continue same Codex thread, summarize, ignore, background Codex Desktop-only completion reminder, and button/command fallback are covered by tests. | Desktop GUI-only state remains outside the current IPC snapshot boundary. |
...
| 23.4 Notification acceptance | Done for personal use | Completion/failure/approval outbox exists; progress is card-updated instead of spammed; notification levels `all/important/errors/muted` are enforced; Codex Desktop-only unbound completed/failed/interrupted turns are scanned and reminded once in the control chat. | @ creator policy is intentionally omitted. |
...
| 28 Open-source absorption upgrade | Partial | Desktop IPC bridge, queue, outbox, semantic events, task detail page, and workspace checkpoints absorbed in a non-Git personal-use form. | QR pairing and full live mirror are not absorbed. |
...
| 31.2 CodexClient wrapper | Done | `CodexClient` wraps ordinary Desktop IPC methods and pagination. | More methods can be added as needed. |
...
| 34.1 Basic Codex Desktop interop | Partial | Uses ordinary Desktop persisted threads visible from IPC snapshots. | Does not mirror GUI state directly. |
| 34.2 Import existing tasks | Partial | `thread/list/read` claim works; unbound recent terminal turns can proactively remind the Feishu control chat for later claim/summary/ignore. | Rollout/session file import is missing. |
...
| 34.4 Desktop owner routing | Done for current scope | `desktop_ipc` mode observes Desktop snapshots and routes follower requests to the Desktop owner `sourceClientId`. | Rename/archive still need confirmed IPC surface. |
| 34.5 Refresh/new thread | Partial | Bridge observes ordinary Desktop IPC snapshots and continues existing Desktop threads through follower requests. Real Feishu-origin new-thread creation is supported only when `desktop_proxy` is connected; otherwise the product keeps a draft and points users to `/tasks`. | More Desktop Proxy diagnostics and runtime availability checks are still needed. |
...
| 42.3 P2 advanced | Mostly not started | P2 boundaries documented. | Desktop owner expansion, multi-machine, teams. |
...
| 44.7 Codex Desktop interop | Partial | Existing idle ordinary Desktop sessions can be discovered, reminded on completion when unbound, resumed, continued, and archived on completion. | Rollout import and Desktop GUI visibility not implemented. |
...
| `diagnostic_recover` | Done; rechecks/ensures Desktop IPC readiness, resets dead outbox, reconciles bindings, then sends diagnosis |
...
/doctor ok: Desktop runtime connected, codexAvailable=true, feishuConfigured=true
...
2. Runtime schema validation using generated Codex schema/types.
...
7. True P2 Desktop owner expansion and multi-machine support.
