...
The project currently implements the main P0 bridge kernel: Feishu long-connection message transport by default, button-first hybrid interaction with message-command fallback for local-only deployments, optional HTTP callback fallback, Codex app-server stdio transport, SQLite persistence, session binding, semantic events, projections, queueing, approvals, outbox retry, diagnostics, incoming-message dedupe, and basic safety controls.
...
| 14 Feishu integration | Partial | Long connection is default for messages and newer `card.action.trigger` card callbacks; message commands cover local-only fallback controls; HTTP callback is optional fallback for cards; message parsing, card action parsing, send/reply/update APIs exist. | Explicit Feishu topic creation remains missing; card button runtime requires app-side `card.action.trigger` subscription. Some tenants may still require HTTP reachability for older card callback wiring. |
